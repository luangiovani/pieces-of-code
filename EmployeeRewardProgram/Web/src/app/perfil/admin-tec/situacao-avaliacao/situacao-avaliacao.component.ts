...
@Component({
  selector: 'app-situacao-avaliacao',
  templateUrl: './situacao-avaliacao.component.html'
})

export class SituacaoAvaliacaoComponent implements OnInit {
  ...
  constructor(public elementRef: ElementRef,
      ...
      this.title = 'Situações de Avaliação';
      initApp();
   }

  ngOnInit() {
    try {
      this.service.ObterSituacoesAvaliacao()
        .subscribe((resp) => {
          if (resp.success) {
            ...
            this.chRef.detectChanges();
            this.dTb.ajusta_toolbar('situacao-avaliacao-novo/', this.elementRef, this.router, this.location);
            this.chRef.detectChanges();
            ...
            Swal.fire({
              type: 'error',
              title: 'Oops...',
              text: 'Houve um erro ao tentar Obter as Situações de Avaliação cadastradas!'
            });
          }
        });
    } catch (err) {
        ...
        this.service.AtivarInativar(
          ...
          .subscribe((recModel) => {
            if (recModel.success) {
              ...
              .then((res) => {
                window.location.href = 'situacao-avaliacao';
              });
            } else {
              Swal.fire({
  ...
  editar(id: string) {
    this.router.navigate(['situacao-avaliacao-novo/', id]);
  }
}
