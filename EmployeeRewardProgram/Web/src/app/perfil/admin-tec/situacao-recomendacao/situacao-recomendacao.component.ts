...
@Component({
  selector: 'app-situacao-recomendacao',
  templateUrl: './situacao-recomendacao.component.html'
})

export class SituacaoRecomendacaoComponent implements OnInit {
  ...
  constructor(public elementRef: ElementRef,
      ...
      this.title = 'Situações de Recomendação';
      initApp();
   }

  ngOnInit() {
    try {
      this.service.ObterSituacoesRecomendacao()
        .subscribe((resp) => {
          if (resp.success) {
            ...
            this.chRef.detectChanges();
            this.dTb.ajusta_toolbar('situacao-recomendacao-novo/', this.elementRef, this.router, this.location);
            this.chRef.detectChanges();
            ...
            Swal.fire({
              type: 'error',
              title: 'Oops...',
              text: 'Houve um erro ao tentar Obter as Situações de Recomendação cadastradas!'
            });
          }
        });
    } catch (err) {
        ...
        this.service.AtivarInativar(
          ...
          .subscribe((recModel) => {
            if (recModel.success) {
              ...
              .then((res) => {
                window.location.href = 'situacao-recomendacao';
              });
            } else {
              Swal.fire({
  ...
  editar(id: string) {
    this.router.navigate(['situacao-recomendacao-novo/', id]);
  }
}
