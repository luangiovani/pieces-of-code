...
@Component({
  selector: 'app-taxa-conversao',
  templateUrl: './taxa-conversao.component.html'
})

export class TaxaConversaoComponent implements OnInit {
      ...
      this.service.ObterTaxas()
        .subscribe((resp) => {
          if (resp.success) {
            ...
            this.chRef.detectChanges();
            this.dTb.ajusta_toolbar('taxa-conversao-novo/', this.elementRef, this.router, this.location);
            this.chRef.detectChanges();
  ...
  situacao(id: string = '', nome: string = '', ativo: string = 'true') {
    ...
    Swal.fire({
      ...
      html: 'Deseja ' + (bAtivo ? 'ativar' : 'inativar' ) + ' a taxa <b>' + nome + '</b>?',
      type: 'question',
          ...
          .subscribe((recModel) => {
            if (recModel.success) {
              Swal.fire({
                ...
                html: 'Taxa gravada no banco de dados',
                type: 'success',
                confirmButtonText: 'OK'
              })
              .then((res) => {
                window.location.href = 'taxa-conversao';
              });
            } else {
              Swal.fire({
  ...
  editar(id: string) {
    this.router.navigate(['taxa-conversao-novo/', id]);
  }
}
