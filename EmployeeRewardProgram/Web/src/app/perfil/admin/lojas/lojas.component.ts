...
export class LojasComponent implements OnInit {
      ...
      this.service.ObterLojas()
        .subscribe((resp) => {
          if (resp.success) {
            ...
            this.chRef.detectChanges();
            this.dTb.ajusta_toolbar('lojas-novo/', this.elementRef, this.router, this.location);
            this.chRef.detectChanges();
            ...
            Swal.fire({
              type: 'error',
              title: 'Oops...',
              text: 'Houve um erro ao tentar Obter as Lojas cadastradas!'
            });
          }
        });
    } catch (err) {
  ...
  situacao(id: string = '', nome: string = '', ativo: string = 'true') {
    ...
    Swal.fire({
      ...
      html: 'Deseja ' + (bAtivo ? 'ativar' : 'inativar' ) + ' a Loja <b>' + nome + '</b>?',
      type: 'question',
  ...
  editar(id: string) {
    this.router.navigate(['lojas-novo/', id]);
  }
