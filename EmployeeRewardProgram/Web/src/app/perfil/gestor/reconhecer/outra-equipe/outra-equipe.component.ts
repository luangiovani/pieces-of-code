...
@Component({
  selector: 'app-outra-equipe',
  templateUrl: './outra-equipe.component.html'
})

export class OutraEquipeComponent implements OnInit {
  ...
  constructor(public elementRef: ElementRef,
      ...
      this.title = 'Colaboradores de Outras Equipes';
  ...
  async ngOnInit() {
    ...
    try {
      await this.colaboradorService.ListarColaboradores(this.currentUser.cs, false)
      .then((resp) => {
        if (resp.success) {
  ...
  reconhecer(cs: string, nome: string) {
    ...
    .then((result) => {
      if (result.value) {
        this.router.navigate(['reconhecer/' + cs + '/false']);
      }
    });
  }
