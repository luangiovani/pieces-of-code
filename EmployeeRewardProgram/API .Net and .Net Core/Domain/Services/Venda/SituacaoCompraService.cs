...
namespace Domain.Services.Venda
{
    ...
    /// </summary>
    public class SituacaoCompraService : BaseService<SituacaoCompra>
    {
        /// <summary>
        /// Objeto do repositório de manipulação do service
        /// </summary>
        private readonly SituacaoCompraRepository _repository;

        /// <summary>
        /// Construtor
        /// </summary>
        /// <param name="repository">Objeto injetado do repositório de banco de dados</param>
        public SituacaoCompraService(SituacaoCompraRepository repository) : base(repository)
        {
        ...
        public SituacaoCompraDTO Gravar(SituacaoCompraDTO dto, string cs_colaborador_logado, Guid logOperacaoId)
        {
            if (dto != null &&
            ...
            {
                SituacaoCompra objSituacao = null;
                if (string.IsNullOrEmpty(dto.id.ToString()))
                {
                    ...
                    if (existente != null)
                    {
                        if (existente.ativo)
                        {
                            throw new Exception("Esta Situação da Compra " + dto.descricao + " já está cadastrada!");
                        }
                        else
                        {
                    ...
                    {
                        objSituacao = new SituacaoCompra()
                        {
                ...
                else
                {
                    ...
                    if (objSituacao != null)
                    {
                        ...
                        throw new InvalidOperationException("Situação da Compra não encontrada para efetuar a atualização.");
                }
            ...
            {
                throw new InvalidOperationException("Os campos para Situação da Compra não foram preenchidos");
            }
        }
    }
}