...
namespace ERwPControllers.Gestao
{
    [Route("[controller]")]
    [ApiController]
    public class SituacaoRecomendacaoController : BaseController
    {
        /// <summary>
        /// Objeto local de Serviço de operações relacionadas a situações de recomendação
        /// </summary>
        private readonly SituacaoRecomendacaoService _service;

        /// <summary>
        /// Construtor
        /// </summary>
        /// <param name="service">Injeção do Serviço de manipulação das Situações de Recomendação</param>
        /// <param name="logger">Injeção do Serviço de Log</param>
        public SituacaoRecomendacaoController(SituacaoRecomendacaoService service, IHttpContextAccessor httpContextAccessor, LogService logger) : base(httpContextAccessor, logger)
        {
            _service = service;
        }
        ...
        [Route("cadastrar")]
        [Route("atualizar")]
        public IActionResult Post([FromBody]SituacaoRecomendacaoDTO situacaoDTO)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    string msgmLog = string.IsNullOrEmpty(situacaoDTO.id.ToString()) ? "Cadastrar nova Situação de Recomendação" : "Atualizar Situação de Recomendação";
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            log = base._logger.LogInicioOperacao(cs_colaborador_logado, aplicacaoLogada_id, msgmLog, "Ocorreu uma exceção ao tentar inserir/atualizar Situação da Recomendação.");
                            base._logger.LogFimOperacao(log.id, ex.Message);
                        }
        ...
        public IActionResult Post([FromBody] AtivarInativarDTO dto)
        {
            try
            {
                ...
                {
                    string msgmLog = dto.ativar ? "Ativar Situação de Recomendação" : "Inativar Situação de Recomendação";
        ...
        /// <summary>
        /// Consultar Situação de Recomendação pelo Id informado
        /// </summary>
        /// <param name="id">Id da Situação de Recomendação para consultar</param>
        /// <returns>Response Status 200 caso sucesso, StatusCode do Erro com "error" descrição do erro.</returns>
        [HttpGet]
        [Route("consultar")]
        public IActionResult Get(string id)
        {
            try
            {
                ...
                {
                    return Ok(new { success = false, message = "Id não informado para obtenção da Situação de Recomendação." });
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Retorna uma listagem de todas as Situações de Recomendação que estão cadastrados no banco de dados.
        /// </summary>
        /// <returns>Lista de Situações de Recomendação</returns>
        [HttpGet]
        [Route("listar")]
