...
namespace Classes
{
    public class Sit_Atend  // T794_Sit_Atend // 
    {
        #region Atributos
        ...
        public static void InsertAtendSitComTrans(int codSit, int codAtend, int codEscr, int codUsuario, OracleTransaction trans)
        {
            ...
            try
            {
                OracleCommand cmd = DataBase.ExecuteNonQueryCmd(trans, CommandType.StoredProcedure, SPINSERTSTAT, parms);
                cmd.Parameters.Clear();
            }
            catch (Exception ex)
            {
        ...
        public static void InsertAtendSitSemTrans(int codSit, int codAtend, int codEscr, int codUsuario)
        {
            ...
            using (OracleConnection conn = new OracleConnection(DataBase.CONN_STRING))
            {
                ...
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        DataBase.ExecuteNonQuery(trans, CommandType.StoredProcedure, SPINSERTSTAT, parms);
                        trans.Commit();
                    }
