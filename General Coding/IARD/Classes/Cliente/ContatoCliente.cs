...
namespace Classes
{
    public class ContatoCliente  // T014_CONTATO_CLIENTE 
    {
        #region Atributos
        ...
        public static void Ativar(int codigoContato, int codigoCliente)
        {
            ...
            using (OracleConnection conn = new OracleConnection(DataBase.CONN_STRING))
            {
                ...
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        DataBase.ExecuteNonQuery(trans, CommandType.StoredProcedure, SPATIVAR, parms);
                        trans.Commit();
                    }
                    catch (OracleException ex)
                    {
        ...
        /// <returns>Instância do objeto Paginação, contendo um DataReader e o total de registros</returns>
        /// 
        public static Paginacao LoadPagEnquadramento(int codContato)
        {
            Paginacao paginacao = new Paginacao();
            ...
            OracleDataReader dr = DataBase.ExecuteReader(CommandType.StoredProcedure, SPENQUADRAMENTOID, parms);
        ...
        private void IntegraRDStation()
        {
            try
            {
                ...
                using (var rdrCLI = DataBase.ExecuteReader(CommandType.Text, getTpCli))
                {
                    if (rdrCLI.Read())
                    {
