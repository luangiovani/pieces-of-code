...
{
    /// <summary>
    /// Classe de Mapeamento com a Entidade de Banco de Dados - T2204_COND_PAGTO
    /// </summary>
    /// <autor>
    ...
    /// </atividade>
    public class CondicaoPagtoEv
    {
        #region Atributos
        private int codCondPagto;
        private Pessoa codPessoa;
        private string dscDsCond;
        private string logUsuario;

        #endregion

        #region Propriedades
        public string DscDsCond
        {
            get { return dscDsCond; }
            set { dscDsCond = value; }
        }
        ...
        #region Construtores
        public CondicaoPagtoEv()
            : this(-1)
        { }
        public CondicaoPagtoEv(int codCondPagto)
        {
        ...
        private const string SPINSERT = "CondicaoPagtoEv.CondicaoPagtoEvInc";
        private const string SPUPDATE = "CondicaoPagtoEv.CondicaoPagtoEvAlt";
        private const string SPDELETE = "CondicaoPagtoEv.CondicaoPagtoEvDel";
        private const string SPSELECTID = "CondicaoPagtoEv.CondicaoPagtoEvSelId";
        private const string SPSELECTPAG = "CondicaoPagtoEv.CondicaoPagtoEvSelPag";
        #endregion

        #region Parametros Oracle
        private const string PARMCODIGO = "codCondPagto";
        private const string PARMCURSOR = "curCondicaoPagtoEv";
        #endregion
        ...
        public static OracleParameter[] GetParameters()
        {
            ...
            if (parms == null)
            {
                ...
                /*0*/ new OracleParameter(PARMCODIGO, OracleType.Int32, 2, ParameterDirection.InputOutput.ToString()) ,
                    ...
                    new OracleParameter( "dscDsCond", OracleType.VarChar),
        ...
        {
            parms[0].Value = this.codCondPagto;
            parms[1].Value = this.codPessoa.CodPessoa;
            parms[2].Value = this.dscDsCond.ToUpper();
            parms[3].Value = this.logUsuario.ToUpper();
            if (this.codCondPagto < 0)
            {
            ...
            using (OracleConnection conn = new OracleConnection(Context.DataBase.CONN_STRING))
            {
                ...
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        codCondPagto = Convert.ToInt32(cmd.Parameters[PARMCODIGO].Value);
        ...
        /// </summary>
        /// <param name="codigo">Código do Registro</param>
        /// <returns>CondicaoPagtoEv</returns>
        public static CondicaoPagtoEv GetDataRow(int codigo)
        {
            OracleDataReader dr = LoadDataDr(codigo);
            CondicaoPagtoEv CondicaoPagtoEv = new CondicaoPagtoEv();
            try
            {
                if (dr.Read())
                {
                    CondicaoPagtoEv.codCondPagto = Convert.ToInt32(dr["A2204_cd_cond_pgto"]);
                    if (dr["A052_cd_usuario"] != DBNull.Value)
                    { CondicaoPagtoEv.codPessoa = new Pessoa(Convert.ToInt32(dr["A052_cd_usuario"])); }
                    if (dr["A2204_ds_cond"] != DBNull.Value)
                    { CondicaoPagtoEv.dscDsCond = Convert.ToString(dr["A2204_ds_cond"]); }
                    if (dr["usu_inc_alt"] != DBNull.Value && dr["usu_inc_alt"].ToString() != "")
                        CondicaoPagtoEv.logUsuario = Convert.ToString(dr["usu_inc_alt"]);
                }
            }
            catch (Exception ex)
            {
                if (!dr.IsClosed)
                    dr.Close();
                CondicaoPagtoEv = new CondicaoPagtoEv();
                throw (ex);
            }
            finally
            {
        ...
        /// <param name="trans">OracleTransaction</param>
        /// <returns>CondicaoPagtoEv</returns>
        public static CondicaoPagtoEv GetDataRow(int codigo, OracleTransaction trans)
        {
