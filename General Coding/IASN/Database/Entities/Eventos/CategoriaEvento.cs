...
{
    /// <summary>
    /// Classe de Mapeamento com a Entidade de Banco de Dados - T911_CATEGORIA
    /// </summary>
    /// <autor>
    ...
    public class CategoriaEvento
    {
        ...
        public string TpoCategoria
        {
            get { return tpoCategoria; }
            set { tpoCategoria = value; }
        }
        ...
        public CategoriaEvento()
            : this(-1)
        { }
        public CategoriaEvento(int codCategoria )
        {
        ...
        private const string SPINSERT = "CategoriaEvento.CategoriaEventoInc";
        private const string SPUPDATE = "CategoriaEvento.CategoriaEventoAlt";
        private const string SPDELETE = "CategoriaEvento.CategoriaEventoDel";
        private const string SPSELECTID = "CategoriaEvento.CategoriaEventoSelId";
        private const string SPSELECTPAG = "CategoriaEvento.CategoriaEventoSelPag";
        #endregion

        #region Parametros Oracle
        private const string PARMCODIGO = "codCategoria";
        private const string PARMCURSOR = "curCategoriaEvento";
        #endregion
        ...
        public static OracleParameter[] GetParameters()
        {
            ...
            if (parms == null)
            {
                ...
                /*2*/ new OracleParameter( "nomCategoria", OracleType.VarChar),
                /*3*/ new OracleParameter( "tpoCategoria", OracleType.VarChar),
        ...
        {
            parms[0].Value = this.codCategoria;
            parms[1].Value = this.CodEvento.CodEvento;
            parms[2].Value = this.nomCategoria.ToUpper();
            parms[3].Value = this.tpoCategoria.ToUpper();
            ...
            using (OracleConnection conn = new OracleConnection(Context.DataBase.CONN_STRING))
            {
                ...
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        codCategoria  = Convert.ToInt32(cmd.Parameters[PARMCODIGO].Value);
        ...
        /// </summary>
        /// <param name="codigo">Código do Registro</param>
        /// <returns>CategoriaEvento</returns>
        public static CategoriaEvento GetDataRow(int codigo)
        {
            ...
            try
            {
                if (dr.Read())
                {
                    CategoriaEvento.codCategoria  = Convert.ToInt32(dr["A911_cod_categoria"]);
                    CategoriaEvento.codEvento = new EventoCredenciamento(Convert.ToInt32(dr["A022_cd_ev"]));
                    if (dr["A911_nom_categoria"] != DBNull.Value && dr["A911_nom_categoria"] != DBNull.Value && dr["A911_nom_categoria"].ToString() != "")
                        CategoriaEvento.nomCategoria = Convert.ToString(dr["A911_nom_categoria"]);
                    if (dr["A911_tpo_categoria"] != DBNull.Value && dr["A911_tpo_categoria"] != DBNull.Value && dr["A911_tpo_categoria"].ToString() != "")
                        CategoriaEvento.tpoCategoria = Convert.ToString(dr["A911_tpo_categoria"]);
                    if (dr["usu_inc_alt"] != DBNull.Value && dr["usu_inc_alt"] != DBNull.Value && dr["usu_inc_alt"].ToString() != "")
                        CategoriaEvento.logUsuario = Convert.ToString(dr["usu_inc_alt"]);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="trans">OracleTransaction</param>
        /// <returns>CategoriaEvento</returns>
        public static CategoriaEvento GetDataRow(int codigo, OracleTransaction trans)
        {
